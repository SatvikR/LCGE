				      ...
				      float height)
{
	...
	LCGE_index_buffer *ib =
		lcge_index_buffer_create(6 * sizeof(GLuint), indeces);
...
LCGE_EXPORT void lcge_rect_rotate(LCGE_rect *rect, float angle)
{
	...
	LCGE_coordinate top_l = lcge_coordinate_rotate(
		rect->top_l.x, rect->top_l.y, center.x, center.y, angle);
	LCGE_coordinate top_r = lcge_coordinate_rotate(
		rect->top_r.x, rect->top_r.y, center.x, center.y, angle);
	LCGE_coordinate bottom_l = lcge_coordinate_rotate(
		rect->bottom_l.x, rect->bottom_l.y, center.x, center.y, angle);
	LCGE_coordinate bottom_r = lcge_coordinate_rotate(
		rect->bottom_r.x, rect->bottom_r.y, center.x, center.y, angle);
	GLfloat positions[8] = {
		bottom_l.x, bottom_l.y, // bottom left
		top_l.x,    top_l.y, // top left
		top_r.x,    top_r.y, // top right
